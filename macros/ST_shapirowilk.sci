...
function [normal, W] = ST_shapirowilk(v, p)   
    ...
    // normal: Returns %T if the null hypothesis of normality is not rejected, otherwise %F.
    // W: Shapiro-Wilk W test statistic (optional second output).
    //
    // Description
    ...
    //
    // The test can be used for sample sizes from 3 to 50 values. It returns a boolean 
    // (%T or %F) whether the dataset is normally distributed or not and as a second 
    // return value (W) the test statistic. The value W is particularly useful because it allows 
    // to see exactly how well or poorly a dataset passes or fails the normality test, 
    // rather than just seeing %T or %F.
    //
    ...
    function [critval] = shapirowilk_crit(n, p)
        ...
        // and alpha (level of significance). Values are tabulated for every
        // sample size n = 3..50; no interpolation is required.
        ...
        // Set the proper column of watable, depending on confidence level
        select(p)
        case("90%")
            j = 2;
        case("95%")
        ...
        case(0.01)
            ...
            error("ST_shapirowilk: Unsupported confidence/significance level.");
        end

        // Exact tabulated value for n = 3..50
    ...
    function alpha = shapirowilk_a(i,n)
        ...
        6	  %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    0.0000	0.0303	0.0539	0.0727	0.0880	0.1005	0.1109	0.1197	0.1271	0.1334	0.1399	0.1443	0.1480	0.1512	0.1539	0.1563	0.1584	0.1601	0.1616	0.1630	0.1641	0.1651	0.1660	0.1667	0.1673	0.1678	0.1683	0.1686	0.1689	0.1691	0.1693	0.1694	0.1695	0.1695	0.1695	0.1695	0.1695	0.1693	0.1692	0.1691; ...
        ...
        17	%nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    0.0000	0.0062	0.0119	0.0172	0.0220	0.0264	0.0305	0.0343	0.0379	0.0411	0.0442	0.0471	0.0497	0.0522	0.0546	0.0568	0.0588	0.0608; ...
        18	%nan	    %nan	    %nan     %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan     %nan   	%nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan   	%nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    %nan	    0.0000	0.0057	0.0110	0.0158	0.0203	0.0244	0.0283	0.0318	0.0352	0.0383	0.0412	0.0439	0.0465	0.0489	0.0511	0.0532; ...
    ...
    if (n < 3 | n > 50) then
        ...
        "sample sizes from 3 to 50 values.")); 
    end
    ...
    // Constant data have zero variance; W is undefined.
    if s2 == 0 then
        error("ST_shapirowilk: All observations are identical; Shapiro-Wilk W is undefined.");
    end

    // Calculate b for even or odd n
    b = 0;
    ...
    // if W is greater than W_alpha, the null hypothesis of normality is not rejected
